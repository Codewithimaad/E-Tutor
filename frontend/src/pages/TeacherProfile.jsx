...
const TeacherProfile = () => {
    ...
    const enrollStudent = async () => {
        try {
            ...
            if (!token) {
                return toast.error("You must be logged in to enroll.");
            }

            if (!userId) {
                return toast.error("User ID is missing from context.");
            }

            if (!teacher || !teacher._id) {
                return toast.error("Teacher ID is missing.");
            }

            const response = await axios.post(
                `${backendUrl}api/enrollments/enroll`,
                {
                    studentId: userId, // ✅ Add this
            ...
            console.log("Enrollment Response:", response.data.message);
            if (response.data.message) {
                toast.success(response.data.message);
            } else {
                toast.error("Enrollment succeeded, but no message returned.");
            }
        } catch (error) {
