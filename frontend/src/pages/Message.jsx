...
const Message = () => {
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            ...
            try {
                const res = await axios.get(`${backendUrl}api/users/get-students`, {
                ...
                setError(prev => ({
                    ...prev,
                    users: err.response?.data?.message || "Failed to load students"
                }));
            } finally {
                    ...
                    <div className="overflow-y-auto">
                        {students.map((student) => (
                            <div
                                ...
                                className={`p-3 rounded-lg mb-2 cursor-pointer flex items-center ${activeChat?._id === student._id
                                    ? "bg-orange-100"
                                    : "hover:bg-gray-100"
                                    }`}
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-sm truncate text-gray-500">
                                        {userStatuses[student._id]?.online
                                            ? <span className="text-green-500">Online</span>
                                            : <span>Last seen {formatLastSeen(userStatuses[student._id]?.lastSeen)}</span>
                                        }
                                    </p>
                                </div>
                            </div>
